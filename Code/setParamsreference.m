...
%{
%dxmin= 24;%8; %m, smallest dx
  %dxmax = 113;%24; %m, largest dx
  % high res dxmin=5.5556;  dxmax=26.1574;
xfac=.6347;
shelfthickness = 100;
dxmin= 24/xfac;%8; %m, smallest dx
 dxmax = 113/xfac;%24; %m, largest dx
aspect=0.1;
...
xloc_stretch = (halfwidth+dxmax*9)/dxmin;
  ...
  dx = (dxmax-dxmin)/2* (tanh(0.05* (xind-xloc_stretch))+1) + dxmin;
  dx = [flip(dx) dx];
...
shelfthickness = 100;
dxmin= 24/xfac;%8; %m, smallest dx
 dxmax = 113/xfac;%24; %m, largest dx %113/xfac
aspect= 0.1;
  halfwidth= (shelfthickness/aspect)/2;
xloc_stretch = (halfwidth+dxmax*(Nx/40))/dxmin; %15
   xind=[1:1:Nx/2];
...
%   [N2 pp_mid] = gsw_Nsquared(SA,CT,pp);
 [N2 pp_mid] = gsw_Nsquared(sRef,tRef,pp);
  ...
  deltaT = round(deltaT);
  deltaT= deltaT/10; %ad hoc: we found that the normal dT wasn't working (approx. dT=14s)
...
uvelfinal = 0.1*cosd(vangle)*ones(Nx,Ny,Nr); %0*ones(Nx,Ny,Nr);
  ...
  temp=ncread('interpolated_data_temp.nc','interpolated_data_temp');
  salt=ncread('interpolated_data_salt.nc','interpolated_data_salt');
  uvel=ncread('interpolated_data_uvel.nc','interpolated_data_uvel');
  vvel=ncread('interpolated_data_vvel.nc','interpolated_data_vvel');
  wvel=ncread('interpolated_data_wvel.nc','interpolated_data_wvel');
  %read input fields from low resolution interpolated to high resolution fields: temp, salt, uvel, vvel
  hydroTh=temp; hydroSa= salt; vvelinitial= vvel; uvelinitial=uvel;
...
scaling_factor = 3.2; % Determines length of top of channel
